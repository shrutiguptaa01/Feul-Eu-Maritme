...
class ComplianceRepositoryPostgres {
    ...
    async getComplianceForYear(year) {
        const res = await this.db.query(`SELECT ship_id AS "shipId", cb_gco2eq AS "cbGco2eq", year 
       FROM ship_compliance 
       WHERE year = $1`, [year]);
        return res.rows;
    }
    // Get CB for a single ship in a year
    async getComplianceRecord(shipId, year) {
    ...
    async getComplianceForShips(shipIds, year) {
        const res = await this.db.query(`
      SELECT ship_id, year, cb_gco2eq
      FROM ship_compliance
